...
// R12 车道 A:顶栏主按钮「生成交付包」迁为固定 AX 名「流水线下一步」(可见文案随步变化)。
const TOPBAR_BUTTONS = ["导入素材", "切换集", "流水线下一步", "设置"];
...
// ─────────────────────────────────────────────────────────────────────────────
{
  // R12:顶栏没有「生成交付包」了,按导航条「第 4 步 导出」开导出抽屉(主按钮的去向随流水线变)。
  const opened = clickByLabel("第 4 步 导出");
  sleep(1200);
