...
## 0.5 业主拍板(09-19 晚):**照片是独立板块,不与视频混排**

原 W1 PH-03 按「同一集里混排、照片当 3 s 段进镜头带」做了,业主看到后明确:视频与照片是**两个独立板块**。改为:
- 同一个「集」下两个工作台 tab:**视频工作台**(0.10.2 原样,零改动)与 **照片工作台**(新壳:照片网格 + 相似组折叠 + 疑似废片 + A/B 擂台 + 检查器 + 评级/收藏/拒绝 + 一句话挑片(照片版));顶栏/首页切换,记住上次所在。
- 照片**不进镜头带、不占 3 s、不计入视频自动挑选预算**;`photo_meta.hold_ms` 只在交付说明里用。
- 交付:剪映素材包 / 整包交付 = `视频/`(原顺序)+ `照片/`(winners 按拍摄时间或相似组顺序,`顺序.txt` 各一份);「导出视频文件」只管视频,不再需要「遇照片阻止」。
- W1 已做的混排 UI(池混排、带照片段、监视器静态检视)退为照片工作台内部组件:监视器静态检视复用;池混排与带照片段**移除**(视频池按 `kind='video'` 过滤)。
- 发动机层(入库/解码/EXIF/伴随/分组/废片/擂台/导出)不变。
- 0.11.0 = W1 发动机 + W2(擂台/分组)+ 照片工作台一起发;不发混排版。

## 0.6 业主拍板(09-20 晚):**照片线不套视频逻辑,照片不交剪映**

- 照片工作台唯一导出 = 「导出精选照片」到文件夹(winners + 原件/伴随 + `顺序.txt`,走归档日志);没有「交给剪映」/素材包/整包交付,三卡在照片工作台不渲染。
- 剪映素材包 / 整包交付回归纯视频,不再生成 `照片/`(§0.5 里的 `视频/`+`照片/` 作废)。
- 从视频抄来的概念在照片侧一律去掉:时长预算(照片按张数)、3 s hold 露出、章节/镜头带/精选段/交付/剪映/快速导出字样;`terminology.test` 加照片工作台词表。
- 0.11.1 = W3(RAW 检视、归档日志、画质真值)+ 本条。
...
| PH-03 池/带/监视器 | **已被 §0.5 取代**:改为 PH-03b 照片工作台(独立板块);监视器静态检视保留复用 | M | 选中照片监视器 ≤ 100 ms 出图;混排截图基线 +2;旧视频路径截图 diff 0 |
