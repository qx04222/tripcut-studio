...
## 6. 业主晚间三条 + 更新路径的严重发现(18:00–18:45)

- **别人电脑也能自动升级**:端点是包内写死的公网地址 `https://github.com/qx04222/tripcut-studio/releases/latest/download/latest.json`;真 GitHub 端点(无任何本机覆盖)实测:0.8.0 发布包「检查更新 → 下载并安装」升到 0.8.1 **通**;0.8.1 发布包零点击自动收到 0.8.2、`updater.last_check` 落库 **通**。
- **顶栏齿轮旁「新版本」提醒**(`UpdateTopChip`):有新版本 → 「新版本 x」点击下载;下载中 → 进度;已下载 → 「重启完成更新」。真机截图 `r083-toast.png`。**设置图标改齿轮** 同图。
- **严重发现(已修,0.8.3)**:0.8.1 / 0.8.2 下载完成后 Tauri `download_and_install` 在运行中原地替换 .app → 界面整个冻住(真鼠标点「开始一个新旅程」/ 齿轮,截图 `cmp` 逐字节相同;AX 树消失;主线程空转);同一 bundle + 新 profile 的对照组正常。修:前端只调 `download_update`(暂存),替换只在 `restart_to_update` / `RunEvent::Exit`。0.8.3 实验室:下载后 bundle mtime 不变、设置可打开(28 个 AX 元素)、toast + 顶栏 chip 都在;按「重启完成更新」→ bundle 替换、进程重启、新实例 17 个 AX 元素。
- **对已装 0.8.1/0.8.2 的用户**:收到 0.8.3 时会卡住一次,退出再打开即是新版本(更新说明与 README 已写)。
- 发布:v0.8.2(P0 两条 + 顶栏提醒 + 齿轮 + 跨集移动 + 旧壳删除 + A16 修复)、v0.8.3(暂存式更新)。均 23 资产、公开仓已同步。
