...
## 4. 0.9.1 真机补验(02:35,发布包,walk8 profile 升级)

| 项 | 结果 |
|---|---|
| 升级首启 | 媒体池 6 s 出卡;时刻分 v3 全量重算 27 条在启动后 10 s 内 done(本机 M5;大库耗时按 27 条 <10 s 线性外推)✅ |
| 「打开方式」/ Dock 拖入路径(`open -a <app> <file>`) | 新建导入批次(3 → 4),内容与库内重复的文件按哈希去重、不加素材 ✅;整窗拖放(鼠标拖 Finder 文件)无法用 AX 脚本模拟,留业主手试 |
| 菜单栏 / Dock 图标 | 全屏截图:菜单栏「旅剪工作台 文件 编辑 显示 窗口 帮助」中文;Dock 里新 squircle 图标与系统图标同大 ✅ |
| Dock 进度 | 未验:短素材任务 <10 s 完成,截图时已无进度;需长素材 |
| 卷弹出 / iCloud | 未验(需外置盘 / iCloud 文件) |

备注:`open -a <app> ""`(空参数)会把当前目录当文件夹导入 —— 与手动拖入任意文件夹行为一致,不算缺陷,但导入前的「这个文件夹里没有视频」提示值得下轮补。
