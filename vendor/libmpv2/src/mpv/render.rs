...
impl Drop for RenderContext<'_> {
    fn drop(&mut self) {
        // TripCut patch: detach the mpv-side update callback BEFORE freeing the
        // context and BEFORE dropping the boxed closure. Upstream freed the
        // closure first, so mpv's vo thread could invoke a dangling callback
        // during shutdown (observed SIGSEGV in ru_wrapper on the vo thread).
        unsafe {
            libmpv2_sys::mpv_render_context_set_update_callback(
                self.ctx,
                None,
        ...
        // TripCut patch: deliberately LEAK the boxed callback instead of
        // freeing it. Even with the NULL+free ordering above, mpv's vo thread
        // was still observed invoking a just-freed callback during teardown
        // (SIGSEGV in ru_wrapper). The closure is ~tens of bytes per player
        // session; leaking it removes the use-after-free entirely.
        if let Some(update_callback_cleanup) = self.update_callback_cleanup.take() {
            std::mem::forget(update_callback_cleanup);
        }
    }
}
