...
/** R13 §3:首页 —— 大按钮、最近的集、R12 四步卡并入顶部。R19 U-03:大按钮改「新建一集」,模板区撤(见 homeR19.test)。 */
describe("HomeScreen", () => {
  it("region「首页」;四步卡在顶部(与流水线同一套数据);唯一的 primary 是「新建一集」= 当前集为空时打开导入抽屉", async () => {
    pinHome(true);
    render(<HomeScreen />);
    ...
    expect(steps.querySelector("li[aria-current='step']")?.textContent).toContain("选文件夹");
    expect(home.querySelectorAll(".ui-button--primary")).toHaveLength(1);
    const start = within(home).getByRole("button", { name: "新建一集" });
  ...
  // R19 U-03:模板卡三张与「模板确认」两条用例随模板区撤掉而删除(Wave 2 P-09 做实后再回);
  // 「新建一集」的确认流程在 homeR19.test 里钉住。
