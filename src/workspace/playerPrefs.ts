...
/**
 * R11 §3 播放器偏好的进程内小仓:`ui.player.*` 三个布尔键。壳的 store 只把 settings
...
function fromSettings(settings: SettingsMap): PlayerPrefs {
  return {
    startAtBest: readUiBool(settings, "ui.player.start_at_best"),
...
/** 首次调用才真的读 settings 表;失败就保持默认值(全开、不静音)。 */
export function loadPlayerPrefs(): Promise<void> {
  loaded ??= getSettings()
    .then((settings) => {
      snapshot = fromSettings(settings);
      emit();
    })
    .catch(() => undefined);
  return loaded;
...
export function notifyPlayerPref(key: PlayerPrefKey, value: boolean): void {
  ...
  if (key === "ui.player.start_at_best") next.startAtBest = value;
