    ...
    <>
      ...
      <div className="settings-sheet-group">
        ...
        <SettingsRow title="预览画质" htmlFor="settings-preview-quality"
          help={`自动：播放、暂停都看原片，原片真的播放不动（不是一时卡顿、也不是被面板挡住）或省内存机器才改播 1080p 代理（还没生成时先用 540p，生成好自动换上）；高画质：1080p 代理；原片：直接读原文件；性能优先：540p 代理。原片始终只读，导出永远读原片。${settings["performance.proxy_enabled"] === "false" ? "预览用小文件已关闭，所有档位均读取原片。" : ""}`}>
          <Select id="settings-preview-quality" aria-label="预览画质"
            ...
            <option value="auto">自动(推荐):看原片,播放不动才改播 1080p 代理</option>
