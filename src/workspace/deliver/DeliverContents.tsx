...
import { ROUGH_CUT_TARGET_LABELS, formatDuration, roughCutTargetKey, summaryLine, type TargetSecondsOption } from "./deliverModel";
...
export function DeliverContents({ status, includeContactSheet, useJianyingDraft, targetSeconds }: DeliverContentsProps): JSX.Element {
  const rows: ContentRow[] = [
    { icon: "mark-in", name: "精选片段", count: `${status.selected_segment_count} 段 · 帧精确重编码` },
    { icon: "heart", name: "整条收藏", count: `${status.selected_whole_count} 条 · 整条 remux` },
    {
      icon: "play",
      name: "参考粗剪",
      count: `1 条 · ${ROUGH_CUT_TARGET_LABELS[roughCutTargetKey(targetSeconds)]} · 1080p H.264`,
    },
...
const PARTS: ReadonlyArray<{ title: string; body: string }> = [
  { title: "精选片段", body: "打点片段帧精确重编码并回读 PTS;无片段的收藏素材整条 remux。" },
  { title: "参考粗剪", body: "按拍摄时间顺序,统一生成 1080p H.264/AAC 文件。" },
  { title: "镜头表 CSV", body: "UTF-8 BOM,含章节、故事顺序、画面参数、星级、L1 角标和失败备注。" },
  { title: "交付说明", body: "一屏中文说明,告诉你如何把稳定包带入剪映。" },
];

/** 「交付包里有什么」四条说明,折叠段,默认收起,没有 01/02/03 水印(规格 §4.2 第 6 条)。 */
export function DeliverPartsDetails(): JSX.Element {
  return (
