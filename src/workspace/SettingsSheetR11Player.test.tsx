...
async function openAppearance(): Promise<HTMLElement> {
  ...
  await act(async () => {
    // R11 简化专项 #2:外观并入「常用」分区(SettingsSheet.test 有搬迁表)。
    within(dialog).getByRole("tab", { name: "常用" }).click();
    await Promise.resolve();
  });
...
describe("R11 播放器偏好开关", () => {
  it("「选中素材从最精彩处开播」「播完自动播下一条」默认都开;关掉写 ui.player.* 并同步给监视器", async () => {
    ...
    expect(autoAdvance.getAttribute("aria-checked")).toBe("true");
    await loadPlayerPrefs();
    expect(getPlayerPrefs()).toMatchObject({ startAtBest: true, autoAdvance: true });
    await act(async () => {
      startAtBest.click();
      await Promise.resolve();
    });
    ...
    expect(apiMocks.setSetting).toHaveBeenCalledWith("ui.player.auto_advance", "false");
    expect(getPlayerPrefs().autoAdvance).toBe(false);
  });
});
